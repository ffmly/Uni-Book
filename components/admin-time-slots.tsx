...
export default function AdminTimeSlots() {
  ...
  const handleAddTimeSlot = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const exists = timeSlots.some((slot) => slot.time === formData.time && slot.period === formData.period)

      if (exists) {
        toast({
          title: language === "ar" ? "خطأ" : "Error",
          description: language === "ar" ? "وقت الحجز موجود بالفعل" : "Time slot already exists",
          variant: "destructive",
        })
        return
      }
      ...
      toast({
        ...
        description:
          language === "ar"
            ? "حدث خطأ أثناء محاولة إضافة وقت الحجز. يرجى المحاولة مرة أخرى."
            : "An error occurred while trying to add the time slot. Please try again.",
        variant: "destructive",
      })
    }
  }
